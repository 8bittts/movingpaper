...
final class YouTubeDownloader: ObservableObject {
    ...
    /// Pinned yt-dlp release. Bump both fields together via `YTDLPInstaller`.
    nonisolated static var pinnedYTDLPVersion: String { YTDLPInstaller.pinnedVersion }
    nonisolated static var pinnedYTDLPSHA256: String { YTDLPInstaller.pinnedSHA256 }

    nonisolated static func sha256Hex(of data: Data) -> String {
        YTDLPInstaller.sha256Hex(of: data)
    }
    ...
    private func performDownload(youtubeURL: String) async -> DownloadOutcome {
        ...
        guard let ytdlp = await YTDLPInstaller.ensureYTDLP() else {
